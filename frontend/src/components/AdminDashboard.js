...
function AdminDashboard() {
  ...
  const [activeSection, setActiveSection] = useState('dashboard'); // 'dashboard', 'users', 'entries'
  ...
  const fetchData = async () => {
    try {
      ...
      setEntriesData({
        ...
        loading: false,
        error: null,
        totalEntries: entriesResponse.data.totalEntries,
        currentPage: entriesResponse.data.currentPage,
        totalPages: entriesResponse.data.totalPages
      });
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/all-entries', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5000/api/entries/${editFormData.id}`, {
        ...editFormData,
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/entries/${selectedEntry.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handlePageChange = async (newPage) => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/entries?page=${newPage}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
              ...
              {entriesData.entries.slice(0, 10).map(entry => (
                <tr key={entry.id}>
                  ...
                  <td className="status-column">
                    <span className={`status-badge ${entry.loadingUnload.toLowerCase()}`}>
                      {entry.loadingUnload}
                    </span>
                  </td>
                </tr>
      ...
      <aside className={`dashboard-sidebar ${isSidebarOpen ? 'open' : ''}`}>
        ...
        <div className="sidebar-menu">
          ...
          >
            ...
            {isSidebarOpen && <span>Vehicle Entries</span>}
          </button>
          <button 
