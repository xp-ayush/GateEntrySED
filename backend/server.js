...
const db = mysql.createConnection({
  ...
  user: 'admin',
  password: 'Sedl12345', //  MySQL password here
...
app.post('/api/entries', verifyToken, (req, res) => {
  ...
  if (!serialNumber || !driverName || !vehicleNumber || !vehicleType || !source || !timeIn) {
    return res.status(400).json({ 
      message: 'Missing required fields',
      required: ['serialNumber', 'driverName', 'vehicleNumber', 'vehicleType', 'source', 'timeIn']
    });
  }
  ...
  const values = [
    ...
    checkBy || null,
    remarks || null,
    req.user.name, // Always use the current user's name
  ];
...
app.get('/api/entries', verifyToken, (req, res) => {
  ...
  const countQuery = 'SELECT COUNT(*) as count FROM entries';
  ...
  db.query(countQuery, (err, countResult) => {
    ...
    // Then, get entries with user information and their units
    const query = `
      SELECT e.*, GROUP_CONCAT(CONCAT('Unit ', u.unit_number) SEPARATOR ', ') as user_units
      FROM entries e
      LEFT JOIN users usr ON e.userId = usr.id
      LEFT JOIN units u ON e.userId = u.userId
      GROUP BY e.id
    ...
    db.query(query, [limit, offset], (err, results) => {
      ...
      // Format the results to include units in recordedBy if available
      const formattedResults = results.map(entry => ({
        ...entry,
        recordedBy: entry.user_units 
          ? `${entry.recordedBy} (${entry.user_units})`
          : entry.recordedBy
      }));
